 ...
 */
public class AmzBestSellersParser implements IParser {
    @Override
    public Elements getList(Document document) {
        return document.select("div.zg_itemRow div.zg_item_compact:first-child div.a-fixed-left-grid-col.a-col-right");
    }

    private Pattern patternPage = Pattern.compile("zg_bs_116169071_pg_(\\d+)\\?");
    ...
    public String [] parserElement(Element element) {
        ...
        if(null==priceElement) priceElement = element.select("div.a-row span.a-size-base.a-color-price").first();
